...
from models.ofa.unify_transformer import TransformerModel
from models.ofa.unify_transformer_layer import TransformerEncoderLayer, TransformerDecoderLayer
from models.ofa.resnet import ResNet
...
def make_source_bucket_position(bucket_size, max_position=DEFAULT_MAX_SOURCE_POSITIONS):
    context_pos = torch.arange(max_position, dtype=torch.long)[:, None]
    memory_pos = torch.arange(max_position, dtype=torch.long)[None, :]
    relative_pos = context_pos - memory_pos
    sign = torch.sign(relative_pos)
...
@register_model("unify_transformer_traj")
class TrajTransformerModel(TransformerModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TrajTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TrajTransformerDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
class TrajTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if getattr(args, "source_layernorm_embedding", False):
            self.source_layernorm_embedding = LayerNorm(embed_dim)
        else:
            self.source_layernorm_embedding = None
        ...
        self.source_pos_ln = LayerNorm(embed_dim)
        ...
        source_bucket_size = 20  # TODO
        source_num_rel_dis = 2 * source_bucket_size - 1
        source_rp_bucket = make_source_bucket_position(source_bucket_size)
        self.source_rel_pos_table_list = nn.ModuleList(
            [Embedding(source_num_rel_dis, self.num_attention_heads, zero_init=True) for _ in range(args.encoder_layers)]
        )
        ...
        self.register_buffer("source_rp_bucket", source_rp_bucket)
        ...
        self.data_cfg = S2TDataConfig(Path(args.user_dir) / "fbank_config.yaml")
        self.subsample = Conv1dSubsampler(
            self.data_cfg.input_feat_per_channel * self.data_cfg.input_channels,
            args.upsampler_conv_channels,
            args.encoder_embed_dim,
            [int(k) for k in args.upsampler_conv_kernel_sizes.split(",")],
        )
        '''
    ...
    def train(self, mode=True):
        super(TrajTransformerEncoder, self).train(mode)
    ...
    def get_source_rel_pos_bias(self, x, idx):
        seq_len = x.size(1)
        rp_bucket = self.source_rp_bucket[:seq_len, :seq_len]
        values = F.embedding(rp_bucket, self.source_rel_pos_table_list[idx].weight)
        values = values.unsqueeze(0).expand(x.size(0), -1, -1, -1)
        values = values.permute([0, 3, 1, 2])
        return values.contiguous()
    ...
    def forward_embedding(
        ...
        source_pos_embed: Optional[torch.Tensor] = None,
        ...
        x = embed = self.embed_scale * token_embedding  # shape = [bsz, src_tokens_len, emb_dim]
        if self.entangle_position_embedding and pos_embed is not None:
            x += pos_embed
        if self.type_embedding is not None:
            x += self.type_embedding(src_tokens.new_zeros(x.size()[:2]))  # v = 0, shape = [bsz, src_tokens_len]
        ...
        if source_embed is not None:
            source_x = source_embed = self.embed_scale * source_embed
            if self.entangle_position_embedding and source_pos_embed is not None:
                source_x += source_pos_embed
            if self.type_embedding is not None:
                #source_x += self.type_embedding2(source_embed.new_full(source_x.size()[:2], fill_value=2))
                source_x += self.type_embedding(source_embed.new_zeros(source_embed.size()[:2]).int())  # v = 2, shape = [bsz, src_tokens_len]
            if self.source_layernorm_embedding is not None:
                source_x = self.source_layernorm_embedding(source_x)
            if self.quant_noise is not None:
                source_x = self.quant_noise(source_x)
            x = torch.cat([source_x, x], dim=1)  # shape = [bsz, src_tokens_len + window_len, emb_dim]
            embed = torch.cat([source_embed, embed], dim=1)  # shape = [bsz, src_tokens_len + window_len, emb_dim]
    ...
    def forward_scriptable(
        ...
        #source_feature, source_feature_length = self.subsample(sources, source_lengths)

        bsz, window_len = sources.shape[:2]
        source_position_idx = utils.new_arange(sources, bsz, window_len)  # shape = [bsz, window_len]
        source_pos_embed = self.embed_positions(source_position_idx)  # shape = [bsz, window_len, emb_dim]

        source_embed = self.emb_sources(sources)  # shape = [bsz, window_len, emb_dim]
        ...
        encoder_padding_mask = src_tokens.eq(self.padding_idx)  # shape = [bsz, n_src_tokens]
        if sources is not None:
            encoder_padding_mask = torch.cat([source_masks, encoder_padding_mask], dim=1)  # shape = [bsz, n_src_tokens, emb_dim]
        if patch_images is not None:
        ...
        x, encoder_embedding = self.forward_embedding(
            src_tokens, source_embed, image_embed, image_embed_2, token_embeddings, pos_embed, source_pos_embed, image_pos_embed, image_pos_embed_2
        )

        has_pads = (src_tokens.device.type == "xla" or encoder_padding_mask.any())
        # account for padding while computing the representation
        if has_pads:
            x = x * (1 - encoder_padding_mask.unsqueeze(-1).type_as(x))  # shape = [bsz, src_tokens_len + window_len, emb_dim]

        # B x T x C -> T x B x C
        x = x.transpose(0, 1)  # shape = [src_tokens_len + window_len, bsz, emb_dim]

        pos_embed = self.pos_ln(pos_embed)
        if sources is not None:
            source_pos_embed = self.source_pos_ln(source_pos_embed)
            pos_embed = torch.cat([source_pos_embed, pos_embed], dim=1)  # shape = [bsz, src_tokens_len + window_len, emb_dim]
        if patch_images is not None:
        ...
        ).transpose(1, 2) * self.pos_scaling  # shape = [bsz, num_attention_heads, src_tokens_len + window_len, emb_dim/num_attention_heads]
        pos_k = self.pos_k_linear(pos_embed).view(
            x.size(1), x.size(0), self.num_attention_heads, -1
        ).transpose(1, 2)  # shape = [bsz, num_attention_heads, src_tokens_len + window_len, emb_dim/num_attention_heads]
        abs_pos_bias = torch.matmul(pos_q, pos_k.transpose(2, 3))  # shape = [bsz, num_attention_heads, src_tokens_len + window_len, src_tokens_len + window_len]
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if src_tokens.size(1) > 0:
                self_attn_bias[:, :, -src_tokens.size(1):, -src_tokens.size(1):] += self.get_rel_pos_bias(src_tokens, idx)  # shape = [bsz, num_attention_heads, src_tokens_len, src_tokens_len]
            if sources is not None:
                # TODO
                self_attn_bias[:, :, -source_embed.size(1):, -source_embed.size(1):] += self.get_source_rel_pos_bias(source_embed, idx)  # shape = [bsz, num_attention_heads, window_len, window_len]
            if patch_images_2 is not None:
            ...
            self_attn_bias = self_attn_bias.reshape(-1, x.size(0), x.size(0))  # shape = [bsz*num_attention_heads, src_tokens_len + window_len, src_tokens_len + window_len]
...
class TrajTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def get_rel_pos_bias(self, x, idx):
        seq_len = x.size(1)
        return self.get_rel_pos_bias_by_size(seq_len, idx)

    def get_rel_pos_bias_by_size(self, seq_len, idx):
        rp_bucket = self.token_rp_bucket[:seq_len, :seq_len]
        values = F.embedding(rp_bucket, self.token_rel_pos_table_list[idx].weight)
    ...
    def get_image_rel_pos_bias(self, x, idx):
        seq_len = x.size(1)
        return self.get_image_rel_pos_bias_by_size(seq_len, idx)

    def get_image_rel_pos_bias_by_size(self, seq_len, idx):
    ...
    def get_pos_info(self, tokens, tgt_pos_embed, src_pos_embed=None, use_image=False):
        ...
        return self.get_pos_info_by_size(batch_size, tgt_len, tgt_pos_embed, src_pos_embed, use_image)

    def get_pos_info_by_size(self, batch_size, tgt_len, tgt_pos_embed, src_pos_embed=None, use_image=False):
    ...
    def forward(
        ...
        if not features_only:
            ...
            x = self.project_embed_to_target(x)  # shape = [bsz, tgt_tokens_len, dim_a]
    ...
    def extract_features_scriptable(
        ...
        bsz, tgt_len = prev_output_tokens.shape[:2]
        ...
        if encoder_out is not None and len(encoder_out["encoder_out"]) > 0:
            enc = encoder_out["encoder_out"][0]  # shape = [src_tokens_len + window_len, bsz, emb_dim]
            assert (
                enc.size()[1] == bsz
            ), f"Expected enc.shape == (t, {bsz}, c) got {enc.shape}"
        if encoder_out is not None and len(encoder_out["encoder_padding_mask"]) > 0:
            padding_mask = encoder_out["encoder_padding_mask"][0]  # shape = [bsz, src_tokens_len + window_len]

        token_position_idx = utils.new_arange(prev_output_tokens, bsz, tgt_len)  # shape = [bsz, tgt_tokens_len]
        tgt_pos_embed = self.embed_positions(token_position_idx)  # shape = [bsz, tgt_tokens_len, emb_dim]
        ...
        self_abs_pos_bias = self.get_pos_info_by_size(bsz, tgt_len, tgt_pos_embed, use_image=False)  # shape = [bsz, num_attention_heads, src_tokens_len + window_len, src_tokens_len + window_len]
        if code_masks is not None and torch.any(code_masks):
            self_image_abs_pos_bias = self.get_pos_info_by_size(bsz, tgt_len, tgt_pos_embed, use_image=True)
        ...
        cross_abs_pos_bias = self.get_pos_info_by_size(bsz, tgt_len, tgt_pos_embed, src_pos_embed=src_pos_embed)  # shape = [bsz, num_attention_heads, src_tokens_len + window_len, src_tokens_len + window_len]
        if code_masks is not None and torch.any(code_masks):
            cross_image_abs_pos_bias = self.get_pos_info_by_size(bsz, tgt_len, tgt_pos_embed, src_pos_embed=src_pos_embed, use_image=True)
            cross_abs_pos_bias[code_masks] = cross_image_abs_pos_bias[code_masks]
        cross_abs_pos_bias = cross_abs_pos_bias.reshape(-1, *cross_abs_pos_bias.size()[-2:])  # shape = [bsz*num_attention_heads, src_tokens_len + window_len, src_tokens_len + window_len]
        ...
        x = self.embed_scale * self.embed_targets(prev_output_tokens)  # shape = [bsz, window_len, emb_dim]
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if code_masks is None or not code_masks.any():
                self_attn_bias += self.get_rel_pos_bias_by_size(tgt_len, idx).unsqueeze(0)
            elif code_masks is not None and code_masks.all():
                self_attn_bias += self.get_image_rel_pos_bias_by_size(tgt_len, idx).unsqueeze(0)
            else:
                self_attn_bias[~code_masks] += self.get_rel_pos_bias_by_size(tgt_len, idx).unsqueeze(0)
                self_attn_bias[code_masks] += self.get_image_rel_pos_bias_by_size(tgt_len, idx).unsqueeze(0)
        ...
        # T x B x C -> B x T x C
        x = x.transpose(0, 1)  # shape = [bsz, window_len, emb_dim]
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        for param_name, param_tensor in self.state_dict().items():
            if (prefix + param_name) not in state_dict:
                state_dict[prefix + param_name] = self.state_dict()[param_name]
                logger.info('not found in checkpoint: prefix=%s, param_name=%s' % (prefix, param_name))
...
@register_model_architecture("unify_transformer_traj", "unify_transformer_traj")
def base_architecture_traj(args):
