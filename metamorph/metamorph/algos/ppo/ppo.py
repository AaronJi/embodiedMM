    ...
    def train(self):
        ...
        for cur_iter in range(cfg.PPO.MAX_ITERS):
            ...
            for step in range(cfg.PPO.TIMESTEPS):
                ...
                obs = next_obs
            print('butter inserted with data of %i steps' % cfg.PPO.TIMESTEPS)
    ...
    def train_on_batch(self, cur_iter):
        ...
        for _ in range(cfg.PPO.EPOCHS):
            ...
            for batch in batch_sampler:
                ...
                #print("losses:", val_loss.data, pi_loss.data, -ent.data, loss.data)
                loss.backward()
                ...
                self.optimizer.step()
            print('    # train on batch: epoch = %i, %i batches' % (j, i))
